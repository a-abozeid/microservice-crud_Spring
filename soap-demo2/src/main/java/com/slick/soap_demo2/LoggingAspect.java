...
public class LoggingAspect {
    ...
    @Before("execution(* com.SlicK.soap_demo2..*(..))")
    ...
    @AfterThrowing(pointcut = "execution(* com.SlicK.soap_demo2..*(..))", throwing = "exception")
    public void logMethodException(JoinPoint joinPoint, Throwable exception) {
        String methodName = joinPoint.getSignature().getName();
        System.out.println("Method: " + methodName + " | Exception: " + exception.getMessage());
    }

    @AfterReturning(pointcut = "execution(* com.SlicK.soap_demo2..*(..))", returning = "result")
    public void logMethodReturn(JoinPoint joinPoint, Object result) {
        String methodName = joinPoint.getSignature().getName();
        System.out.println("Method: " + methodName + " | Returned: " + result);
    }

    @Around("execution(* com.SlicK.soap_demo2..*(..))")
